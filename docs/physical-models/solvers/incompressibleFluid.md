...
## 一句话
不可压、等温单相流体的稳态/瞬态湍流求解模块（可选动网格）。

## 适用条件（求解器 / 场 / 前提）
- **模块类型**: foamRun 求解器模块
- **源码头文件**: [`applications/modules/incompressibleFluid/incompressibleFluid.H`](../../../applications/modules/incompressibleFluid/incompressibleFluid.H)
- **链接库 (Make/options)**: `fluidSolver`
...
**提示**: `constant/physicalProperties`, `constant/momentumTransport`；`0/U`,`0/p`

## 物理包使用情况（基于 Make/options 证据）
- **热物性**: 内建 — `physicalProperties` / `viscosityModel`（无能量方程）
- **动量输运**: 可选 — 经 `fluidSolver` 链接不可压动量输运库
- **热输运**: 不可用
- **辐射**: 不可用
- **燃烧/化学**: 不可用
- **fvModels / fvConstraints**: 可选 — 经基类链接
- **典型场**: `U`, `p`（及湍流辅助场）
...
- [`applications/modules/incompressibleFluid/Make/options`](../../../applications/modules/incompressibleFluid/Make/options)
