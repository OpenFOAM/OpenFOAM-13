...
## 一句话
两相不互溶液体基类（混合物动量；混合物或分相输运）。

## 适用条件（求解器 / 场 / 前提）
- **模块类型**: 基类 / 支撑模块（通常不作为 controlDict 最终 solver 单独使用，或仅作父类）
- **源码头文件**: [`applications/modules/twoPhaseSolver/twoPhaseSolver.H`](../../../applications/modules/twoPhaseSolver/twoPhaseSolver.H)
- **链接库 (Make/options)**: `VoFSolver`, `physicalProperties`, `interfaceCompression`, `interfaceProperties`, `twoPhaseMixture`, `finiteVolume`, `meshTools`, `fvModels`, `fvConstraints`, `sampling`
...
- **角色**: 两相基类 — 混合物动量；混合物或分相输运（Description）
- **物理包**: 由派生模块决定
...
- [`applications/modules/twoPhaseSolver/Make/options`](../../../applications/modules/twoPhaseSolver/Make/options)
