...
## 一句话
固体域导热（共轭传热/HVAC 固体区域）。

## 适用条件（求解器 / 场 / 前提）
- **模块类型**: foamRun 求解器模块
- **源码头文件**: [`applications/modules/solid/solid.H`](../../../applications/modules/solid/solid.H)
- **链接库 (Make/options)**: `solidThermo`, `solidThermophysicalTransportModels`, `coupledThermophysicalTransportModels`, `specie`, `finiteVolume`, `meshTools`, `sampling`, `fvModels`, `fvConstraints`
...
**提示**: `solidThermo` + `thermophysicalTransport`

## 物理包使用情况（基于 Make/options 证据）
- **热物性**: 内建 — `-lsolidThermo`
- **动量输运**: 不可用
- **热输运**: 内建/可选 — `solidThermophysicalTransportModels`
- **辐射**: 待填
- **燃烧/化学**: 不可用
...
- [`applications/modules/solid/Make/options`](../../../applications/modules/solid/Make/options)
