...
- **部件类型**：`equationOfState` = `icoTabulated`（字典中写短名时可能去掉后缀，见 `thermoType`）
- **合法宏组合**：`forTabulated`.H`
- **适用**: 使用 `fluidThermophysicalModels` / 多组分 / 固体 thermo 的求解器（见矩阵）
- 字典关键字常见写法：`equationOfState` icoTabulated;`（`perfectGas` 等）
...
- 源码：[`src/thermophysicalModels/specie/equationOfState/icoTabulated/icoTabulated.H`](../../../src/thermophysicalModels/specie/equationOfState/icoTabulated/icoTabulated.H)
