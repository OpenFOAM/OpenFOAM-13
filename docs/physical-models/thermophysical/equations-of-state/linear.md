...
- **部件类型**：`equationOfState` = `linear`（字典中写短名时可能去掉后缀，见 `thermoType`）
...
- 字典关键字常见写法：`equationOfState` linear;`（`perfectGas` 等）
...
- 源码：[`src/thermophysicalModels/specie/equationOfState/linear/linear.H`](../../../src/thermophysicalModels/specie/equationOfState/linear/linear.H)
